...
class PostInstall extends AbstractVerticle {

  public static void main(String[] args) {
    Vertx.vertx().deployVerticle(new PostInstall(), deploy -> {
      if (deploy.failed()) {
  ...
  public void start() {
    ...
    final JsonObject npm = new JsonObject(fs.readFileBlocking("package.json"));
    ...
    fs.writeFileBlocking("node_modules/pom.xml", Buffer.buffer(render(data)));
  }
    ...
    for (Map.Entry<String, Object> kv : jsonDependencies) {
      ...
      if (!dependencies.containsKey(kv.getKey())) {
        ...
        if (dir.exists()) {
          ...
          if (file.exists()) {
            final JsonObject json = new JsonObject(fs.readFileBlocking(file.getAbsolutePath()));
