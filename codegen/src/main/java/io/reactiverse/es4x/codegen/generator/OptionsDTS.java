  ...
  public String render(DataObjectModel model, int index, int size, Map<String, Object> session) {
    ...
    for (Map.Entry<String, PropertyInfo> entry : model.getPropertyMap().entrySet()) {
      ...
      if (property.isSetter()) {
        ...
        writer.printf("  %s(%s: %s): %s;\n", property.getSetterMethod(), cleanReserved(entry.getKey()), genType(property.getType(), true), model.getType().getRaw().getSimpleName());
      }
      ...
      if (property.isAdder()) {
        ...
        writer.printf("  %s(%s: %s): %s;\n", property.getAdderMethod(), cleanReserved(entry.getKey()), genType(property.getType(), true), model.getType().getRaw().getSimpleName());
      }
